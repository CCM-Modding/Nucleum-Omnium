...
public abstract class GuiBase extends GuiContainer {
    ...
    protected void mouseClicked(final int x, final int y, final int mouseButton) {
        ...
        final TabBase tab = this.getTabAtPosition(this.mouseX, this.mouseY);

        if (tab != null && !tab.handleMouseClicked(this.mouseX, this.mouseY, mouseButton)) {
            for (final TabBase other : this.tabs)
    ...
    protected void drawElements() {
        ...
        for (final ElementBase element : this.elements)
            element.draw();
    }
    ...
    protected void drawTabs() {
        ...
        for (final TabBase tab : this.tabs) {
            tab.update();
            if (!tab.isVisible())
                continue;
            if (tab.side == 0) {
                tab.draw(this.guiLeft, this.guiTop + yPosLeft);
                yPosLeft += tab.currentHeight;
            } else {
                tab.draw(this.guiLeft + this.xSize, this.guiTop + yPosRight);
    ...
    protected void drawTooltips() {
        ...
        final ElementBase element = this.getElementAtPosition(this.mouseX, this.mouseY);
     ...
     */
    /*
     * public void drawFluid(int x, int y, FluidStack fluid, int width, int
     * height) { int i = 0; int j = 0; int drawHeight = 0; int drawWidth = 0;
     * for (i = 0; i < width; i += 16){ for (j = 0; j < height; j += 16){
     * drawWidth = MathHelper.minI(width - x, 16); drawHeight =
     * MathHelper.minI(height - y, 16); drawScaledTexturedModelRectFromIcon(x +
     * i, y + j, fluid.getFluid().getIcon(fluid), drawWidth, drawHeight); } }
     * GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0F); }
     */
    ...
    public void drawSizedTexturedModalRect(final int x, final int y, final int u, final int v,
        ...
        tessellator.addVertexWithUV(x + 0, y + height, this.zLevel, (u + 0) * texU, (v + height)
                * texV);
        tessellator.addVertexWithUV(x + width, y + height, this.zLevel, (u + width) * texU,
                (v + height) * texV);
        tessellator.addVertexWithUV(x + width, y + 0, this.zLevel, (u + width) * texU, (v + 0)
                * texV);
        tessellator.addVertexWithUV(x + 0, y + 0, this.zLevel, (u + 0) * texU, (v + 0) * texV);
        tessellator.draw();
    }

    public void drawScaledTexturedModelRectFromIcon(final int x, final int y, final Icon theIcon,
            final int width, final int height) {

        final double minU = theIcon.getMinU();
        final double maxU = theIcon.getMaxU();
        final double minV = theIcon.getMinV();
        final double maxV = theIcon.getMaxV();

        final Tessellator tessellator = Tessellator.instance;
        tessellator.startDrawingQuads();
        tessellator.addVertexWithUV(x + 0, y + height, this.zLevel, minU, minV + (maxV - minV)
                * height / 16F);
        tessellator.addVertexWithUV(x + width, y + height, this.zLevel, minU + (maxU - minU)
                * width / 16F, minV + (maxV - minV) * height / 16F);
        tessellator.addVertexWithUV(x + width, y + 0, this.zLevel, minU + (maxU - minU) * width
                / 16F, minV);
    ...
    public void drawTooltip(final String tooltip) {
        ...
        if (tooltip == null || tooltip.equals(""))
            return;
        this.drawCreativeTabHoveringText(tooltip, this.mouseX, this.mouseY);
    }
