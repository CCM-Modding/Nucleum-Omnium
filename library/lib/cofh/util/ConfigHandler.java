...
public class ConfigHandler {
        ...
        for (final ArrayList<String> blockEntrie : this.blockEntries)
            for (final String entry : blockEntrie)
                if (this.modConfiguration.hasKey(Configuration.CATEGORY_BLOCK, entry)) {
                    final int existingId = this.modConfiguration
        ...
        for (final ArrayList<String> blockEntrie : this.blockEntries)
            for (final String entry : blockEntrie)
                if (!this.modConfiguration.hasKey(Configuration.CATEGORY_BLOCK, entry)) {
                    ...
                    for (int j = this.blockIdStart; j < this.blockIdStart + this.blockIdCounter
                            && !idFound; ++j)
        ...
        for (final ArrayList<String> itemEntrie : this.itemEntries)
            for (final String entry : itemEntrie)
                if (!this.modConfiguration.hasKey(Configuration.CATEGORY_ITEM, entry)) {
                    ...
                    boolean idFound = false;
                    for (int j = this.itemIdStart; j < this.itemIdStart + this.itemIdCounter
                            && !idFound; ++j)
                        if (!this.assignedIds.contains(j)) {
    ...
    public boolean renameProperty(final String category, final String key,
        ...
        if (this.modConfiguration.hasKey(category, key)) {
            ...
            if (prop.isIntValue()) {
                final int value = this.modConfiguration.getCategory(category).getValues().get(key)
                        .getInt();
                ...
                final boolean value = this.modConfiguration.getCategory(category).getValues()
                        .get(key).getBoolean(false);
                this.removeProperty(category, key);
                ...
                final double value = this.modConfiguration.getCategory(category).getValues()
                        .get(key).getDouble(0.0);
                this.removeProperty(category, key);
                ...
                final String value = this.modConfiguration.getCategory(category).getValues()
                        .get(key).getString();
                this.removeProperty(category, key);
