...
public class ConfigHandler {
        ...
        for (final ArrayList<String> blockEntrie : blockEntries) {
            for (final String entry : blockEntrie) {
                if (modConfiguration.hasKey(Configuration.CATEGORY_BLOCK, entry)) {
                    final int existingId = modConfiguration.getCategory(Configuration.CATEGORY_BLOCK)
                                                           .getValues()
                                                           .get(entry)
                                                           .getInt();
        ...
        for (final ArrayList<String> itemEntrie : itemEntries) {
            for (final String entry : itemEntrie) {
                if (modConfiguration.hasKey(Configuration.CATEGORY_ITEM, entry)) {
                    final int existingId = modConfiguration.getCategory(Configuration.CATEGORY_ITEM)
                                                           .getValues()
                                                           .get(entry)
                                                           .getInt();
                    assignedIds.add(existingId);
    ...
    public boolean renameProperty(final String category,
                                  final String key,
                                  final String newCategory,
                                  final String newKey,
                                  final boolean forceValue) {
        ...
        if (modConfiguration.hasKey(category, key)) {
            ...
            if (prop.isIntValue()) {
                ...
                final boolean value = modConfiguration.getCategory(category)
                                                      .getValues()
                                                      .get(key)
                                                      .getBoolean(false);
                removeProperty(category, key);
                ...
                final double value = modConfiguration.getCategory(category)
                                                     .getValues()
                                                     .get(key)
                                                     .getDouble(0.0);
                removeProperty(category, key);
