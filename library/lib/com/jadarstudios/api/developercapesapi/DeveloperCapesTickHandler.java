/**
 * Copyright (c) Jadar, 2013 Developer Capes API by Jadar License: Lesser GNU Public License v3
 * (http://www.gnu.org/licenses/lgpl.html) version 1.3.1
 */
...
public class DeveloperCapesTickHandler implements ITickHandler {
	...
	public void tickStart(final EnumSet<TickType> type, final Object... tickData) {
		/*
		 * Will not run if there is no world, and if there are player entities in the playerEntities
		 * list.
		 */
			...
			for (int counter = 0; counter < players.size(); counter++) {
				...
				if (players.get(counter) != null) {
					...
					if (player.cloakUrl.startsWith("http://skins.minecraft.net/MinecraftCloaks/")) {
						/*
						 * Lowers the case of the Username, so that there are no problems with the
						 * Username's case.
						 */
